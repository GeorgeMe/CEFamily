 ...
 */
public class ToyBricksProductParser extends BaseParser<ArrayList<ToyBricksProduct>> {
    @Override
    public ArrayList<ToyBricksProduct> parseJSON(String paramString) throws JSONException,IOException  {
        if (!TextUtils.isEmpty(checkResponse(paramString))){
            Gson gson=new Gson();
            saveResponse(paramString);
            return gson.fromJson(checkResponse(paramString),new TypeToken<ArrayList<ToyBricksProduct>>() {}.getType());
        }
        return null;
    }
    ...
    public void saveResponse(String paramString) throws JSONException ,IOException {
        File file=new File(Environment.getExternalStorageDirectory() +"/.toybricks/"+"product.json");
