...
/**
 * Created by George on 2015/3/28.
 */
public class DownLoadTask implements Runnable {
    ...
    public static final int ERROR_SOCKETTIMEOUT = 3;// 连接超时异常
    ...
    public void run() {
        ...
        try {
            ...
            if (e instanceof ConnectException) {
                ...
                notifyListenerError(ERROR_SOCKETTIMEOUT);
                //Logger.e(TAG, "连接超时", e);
            } else {
            	notifyListenerError(ERROR);
            	//Logger.e(TAG, "未知错误", e);
    ...
    private void start(int length) throws FileNotFoundException {
        ...
        if (threadCount > 1) {
            ...
            for (int i = 0; i < threadCount; i++) {
                ...
                downLoadThread = new DownLoadThread(start, end, new RandomAccessFile(savePath, "rwd"), url, i, queue,buffer);
            ...
            downLoadThread = new DownLoadThread(start, length - 1, new RandomAccessFile(savePath, "rwd"), url, 0,queue, buffer);
            threads.add(downLoadThread);
            downLoadThread.start();
        }
    }
